...
public class SecondFloor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(cm.synced && timeSinceLastOrder >= orderInterval && s)
        {
            ...
            {   
                // Slope(=toio_dict[7])をCube2とくっつける
                if(check == 0 && StartClicked)
                {
                    // 1. InputFieldに入力された値を取得し，PosCube2とAngleCube2に代入し，PosCube7とAngleCube7を計算する
                    if(phase == 0)
                    {
                        int x = int.Parse(InputFieldX.text);
                        int y = int.Parse(InputFieldY.text);
                        int angle = int.Parse(InputFieldAngle.text);
                        PosCube2 = new Vector2(x, y);
                        AngleCube2 = angle;

                        PosCube7 = CalculateNewPosition(PosCube2, AngleCube2 - 90, L);
                        AngleCube7 = AngleCube2 - 90;
                    ...
                    else if(phase == 3)
                    {
                        ...
                        {
                            // 3-2-1. 必要な情報を変数に保存する
                            // 3-2-1-1. Cube2(=toio_dict[0])の位置と角度をそれぞれx1, y1, theta1に保存
                            ...
                            // 3-2-1-3. alpha, betaの値は0, 180とする (左にくっつき，後退してくっつく場合)
                            alpha = 90;
                            beta = 180;
                    ...
                    else if(phase == 8) 
                    {
                        if(navigator.cube.id == toio_dict[7])
                        {
                            float distance = Vector2.Distance(new Vector2(navigator.cube.x, navigator.cube.y), PosCube2);
                            if(distance < 30)
                            {
                                navigator.handle.Stop();
                                phase += 1;
                ...
                else if(check == 3)
                {
                    ...
                    // 4-1. Cube5(=toio_dict[5])がCube2(=toio_dict[2])の上(=toio_pos[2])に乗るまでバックする
                    else if(phase == 3)
                    {
                        if(navigator.cube.id == toio_dict[5])
                        {
                    ...
                    //     if(navigator.cube.id == toio_dict[5])
                    //     {
                    //         int angle_diff = 270 - navigator.cube.angle;
                    ...
                    // 5. Slope(=toio_dict[7])は，0.5s前進する
                    else if(phase == 4)
                    {
                        if(navigator.cube.id == toio_dict[7])
                        {
                ...
                // Slope(=toio_dict[7])をCube1とくっつける
                if(check == 4 && StartClicked)
                {
                    // 1. InputFieldに入力された値を取得し，PosCube2とAngleCube2に代入し，PosCube7とAngleCube7を計算する
                    if(phase == 0)
                    {
                        ...
                        int y = int.Parse(InputFieldY.text);
                        int angle = int.Parse(InputFieldAngle.text);
                        PosCube1 = new Vector2(x, y);
                        AngleCube1 = angle;

                        PosCube7 = CalculateNewPosition(PosCube1, AngleCube1 + 180, L);
                        AngleCube7 = AngleCube1 + 180;
                    ...
                    else if(phase == 3)
                    {
                        if(navigator.cube.id == toio_dict[7])
                        {
                            ...
                            // 3-2-1-3. alpha, betaの値は0, 180とする (後にくっつき，後退してくっつく場合)
                            alpha = 0;
                            beta = 180; // ←これだと前進し始めた
                ...
                else if(check == 5)
                {
                    ...
                    if(phase == 0)
                    {
                        if(navigator.cube.id == toio_dict[7] && navigator.cube.x != 0 && navigator.cube.y != 0)
                        {
                            ...
                            Debug.Log("PosCube5: (" + PosCube4.x + ", " + PosCube4.y + ", " + AngleCube5 + ")");
                            phase += 1;
                        }
                    }

                    // 2. Cube4(=toio_dict[4])をPosCube5まで移動させる
                    else if(phase == 1)
                    {
                    ...
                    // 3-1. Cube4(=toio_dict[4])をAngleCube5の角度へ回転する
                    else if(phase == 2) 
                    {
                        if(navigator.cube.id == toio_dict[4])
                        {
                ...
                // Press(=toio_dict[6])でCube4(=toio_dict[4])を押し出して，Cube4(=toio_dict[4])がCube1(=toio_dict[1])の上に移動する
                else if(check == 6)
                {
                    ...
                    // 3. Cube4(=toio_dict[4])がPosFlat付近(distance<5)までバックする
                    else if(phase == 2)
                    {
                        if(navigator.cube.id == toio_dict[4])
                        {
                    ...
                    // 4-1. Cube4(=toio_dict[4])がCube1(=toio_dict[1])の上(=toio_pos[1])に乗るまでバックする
                    else if(phase == 3)
                    {
                        if(navigator.cube.id == toio_dict[4])
                        {
                ...
                // Cube3(=toio_dict[3])を坂の前へ移動する
                else if(check == 7)
                {
                    // 1. Cube3(=toio_dict[3])の移動先の座標と角度を計算する
                    if(phase == 0)
                    {
                        if(navigator.cube.id == toio_dict[7] && navigator.cube.x != 0 && navigator.cube.y != 0)
                        {
                            ...
                            PosCube3 = CalculateNewPosition(PosCube7, AngleCube7, L_Slope);
                            AngleCube3 = AngleCube7;
                    ...
                    // 2. Cube3(=toio_dict[3])をPosCube3まで移動させる
                    else if(phase == 1)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                            var mv = navigator.Navi2Target(PosCube3.x, PosCube3.y, maxSpd:5).Exec();
                            if(mv.reached)
                            {
                                phase += 1;
                                Debug.Log("phase1");
                    ...
                    // 3-1. Cube3(=toio_dict[3])をAngleCube3の角度へ回転する
                    else if(phase == 2) 
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                            int angle_diff = AngleCube3 - navigator.cube.angle;
                            if(Math.Abs(angle_diff) < 3)
                            {
                                navigator.handle.Stop();
                                phase += 1;
                    ...
                    else if(phase == 3)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                            // 3-2-1. 必要な情報を変数に保存する
                            // 3-2-1-1. Cube7の位置と角度をそれぞれx1, y1, theta1に保存
                            // 3-2-1-2. Cube3(=toio_dict[3])の位置と角度をそれぞれx2, y2, theta2に保存
                            x1 = (int)PosCube7.x;
                            y1 = (int)PosCube7.y;
                            theta1 = AngleCube7;
                    ...
                    else if(phase == 5)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {            
                            // 3-2-4. Cube3(=toio_dict[3])をgammaまで回転させる
                            int angle_diff = gamma - navigator.cube.angle;
                            if(Math.Abs(angle_diff) < 5)
                            {
                                navigator.handle.Stop();
                    ...
                    else if(phase == 6)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                            // 3-2-5. Cube3(=toio_dict[3])をRまで移動させる
                            var distance = Vector2.Distance(new Vector2(navigator.cube.x, navigator.cube.y), R);
                            if(distance < 5)
                            {
                                navigator.handle.Stop();
                    ...
                    else if(phase == 7)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                            // 3-2-6. Cube3(=toio_dict[3])をAngleCube3まで回転させる
                            int angle_diff = AngleCube3 - navigator.cube.angle;
                            if(Math.Abs(angle_diff) < 3)
                            {
                                navigator.handle.Stop();
                ...
                // Press(=toio_dict[6])でCube3(=toio_dict[3])を押し出して，Cube3(=toio_dict[3])がCube0(=toio_dict[0])の上に移動する
                else if(check == 9)
                {
                    // 1-1. Cube3(=toio_dict[3])が坂の入口(PosSlopeStart)に乗るまでバックする
                    // 1-2. Press(=toio_dict[6])もCube3(=toio_dict[3])が坂の入口(PosSlopeStart)に乗るまでバックする
                    if(phase == 0)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                    ...
                    // 3. Cube3(=toio_dict[3])がPosFlat付近(distance<5)までバックする
                    else if(phase == 2)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                    ...
                    // 4. Cube3(=toio_dict[3])がCube0(=toio_dict[0])の上(=toio_pos[0])に乗るまでバックする
                    else if(phase == 3)
                    {
                        if(navigator.cube.id == toio_dict[3])
                        {
                ...
                // Press(=toio_dict[6])とSlope(=toio_dict[7])を逃がす
                else if(check == 10)
                {   
                    // 1. Press(=toio_dict[6])とSlope(=toio_dict[7])を0.5s前進させる
                    if(phase == 0)
                    {
                        if(navigator.cube.id == toio_dict[6] || navigator.cube.id == toio_dict[7])
                        {
                            navigator.handle.Move(100, 0, 100);
                        }
                    ...
                    // 2. checkをインクリメントして，次のPCへ操作するようコンソールにメッセージを表示する
                    else if(phase > 0)
                    {
                        phase = 0;
                        check += 1;
                        Debug.Log("check: " + check);
                        Debug.Log("手順2：PCを変えて，矢印キー(下)を押してください");
                    }
                }
                ...
                foreach(var cube in cm.syncCubes)
                {
                    if(cube.id == toio_dict[3]) text += "toio_dict[3]: " + cube.pos + "\n";
