...
public class MovingOnSlope : MonoBehaviour
{  
    public Text label;
    ...
    void Update()
    {
        ...
        if (cm.synced && timeSinceLastOrder >= orderInterval && s)
        {
            ...
            {
                // 1. Cube[NumCube]とCube[NumPress]の移動後の座標と角度を計算
                if(phase == 0)
                {
                ...
                // 2-1. Cube[NumCube]をPosCubeまで移動
                else if(phase == 1)
                {
                    if(navigator.cube.id == toio_dict[NumCube])
                    {
        ...
        foreach (var cube in cm.syncCubes)
        {
            if (cube.id == toio_dict[NumCube]) 
            {
                text += "Cube[Cube]：(" + cube.pos.x + ", " + cube.pos.y + ")\n";
                text += "Cube[Cube]_Angle：(" + cube.angle + ")\n";
            }
